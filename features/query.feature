Feature: vori query
  ...
  Scenario: Filter by frontmatter tag (exact match)
    ...
    When I run "vori query <vault> --tag status=draft"
  ...
  Scenario: Filter by multiple tags (AND semantics)
    ...
    When I run "vori query <vault> --tag status=draft --tag type=idea"

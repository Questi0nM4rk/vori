...
function main(): void {
	const { command, vaultPath, noteArg, tags, hashtags, json, days, errors } =
	...
	switch (command) {
		...
		case "tags": {
			...
			if (json) {
				...
				if (hashtagCounts.size > 0) {
					process.stdout.write("\n#hashtags:\n");
					for (const [tag, count] of [...hashtagCounts].sort(
						(a, b) => b[1] - a[1],
					)) {
		...
		case "links": {
			if (!noteArg) {
				process.stderr.write(
					"Error: note-name is required for 'links' command.\n",
				);
				process.exit(1);
			}
			const notes = loadVault(absVault);
			const note = notes.find(
				(n) => n.name.toLowerCase() === noteArg.toLowerCase(),
			);
			if (!note) {
		...
		case "backlinks": {
			if (!noteArg) {
				process.stderr.write(
					"Error: note-name is required for 'backlinks' command.\n",
				);
				process.exit(1);
			}
			const notes = loadVault(absVault);
